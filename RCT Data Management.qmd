...
# checking if followup data is empty; if empty, linking is skipped
# this was used at the beginning stages of recruitment
if (!any(is_empty(ctl1_df) | is_empty(ctl2_df) | is_empty(int1_df) | is_empty(int2_df))) {
...
} else { # if follow-ups are empty, baseline dataset is used as df going forward
  df <- baseline_df
}
...
The code below creates a variable (manual_check), flagging participants for manual inspection if they provided a duplicate email, phone number, access codes, or IP addresses. The research team discusses any participants who are flagged by these checks, and once a decision is reached, these decisions are recorded in a CSV file. The code then checks this list, and creates a second variable (manual_decision) un-flagging any participants who have already had a decision made.
...
Manual decisions column can have 4 values indicating different failed checks:

-   0: no problems

-   2: duplicate emails in the "email" and/or "email_end" variables

-   3: duplicate access codes
...
IPs <- read.xlsx("SSMRT Participant Info.xlsx", sheet = "IPs")
...
df$manual_check[df$access_code %in% duplicate_access_code] <- 3
...
df$manual_check[df$record_id %in% (IPs$record_id[IPs$ip %in% duplicate_ips])] <- 4
...
decisions <- read.xlsx("SSMRT Participant Info.xlsx", sheet = "Manual decisions")
...
for (id in decisions$access_code[is.na(decisions$record_id)]) {
  ...
  if (!is.null(id)) { # if "id" is not NULL
    ...
    if (decisions$decision[decisions$access_code == id & !is.na(decisions$access_code)] == "e") { # if the decision is to exclude
...
for (id in decisions$record_id[decisions$access_code == "" | is.na(decisions$access_code)]) {
...
for (id in decisions$access_code[decisions$access_code != "" & 
  ...
  if (!is.null(id)) { # if "id" is not NULL
    ...
    df$manual_decided[df$access_code == id & !is.na(df$access_code)] <- 1 
    # un-exclude participants with that access code and the record id fetched
    df$manual_decided[df$access_code == id & df$record_id == rid
                      & !is.na(df$access_code)] <- 0
...
### Duplicate codes with different email addresses
duplicate_access_codes <- unique(df$codes[duplicated(df$codes) 
                                          & !is.na(df$codes)
                                          & df$codes != "undefinedundefined"])
df$duplicated_codes <- 0

for (item in duplicate_access_codes) {
  email_quant <- length(unique(df$email[df$codes == item & !is.na(df$codes)]))
  if (email_quant != 1) {df$duplicated_codes[df$codes == item] <- 1}
}
...
IP addresses are assigned per device; as such, it's highly unlikely that participants have the same IP address, unless they are 1) family/household members completing the study on the same device, 2) participants attempting to game the system and assuming multiple identities or 3) the same participant attempting the survey multiple times.

As we have no way of identifying participants, it's impossible for us to say which of the above is the case. As such, we flag all duplicate IP addresses for removal, with the exception of duplicate IP addresses where the screening question responses are identical between attempts. In the latter case, it's possible that participants had to exit the survey (e.g., ran out of time) and came back at a later time to complete the survey. In this case, all responses should be approximately the same, particularly in the demographic questionnaire. These participants will instead be manually inspected.
...
# getting duplicate IPs
duplicate_IPs <- unique(IPs$ip[duplicated(IPs$ip)])

df$ip_duplicate <- 0

for (IP in duplicate_IPs) {
  # record_id values with the same IP address
  rid <- IPs$record_id[IPs$ip == IP]
  
  # vector of ages with the same IP address
  ages <- df$age_screen[df$record_id %in% rid]
  # vector of cannabis_screen responses with the same IP address
  cannabis <- df$cannabis_screen[df$record_id %in% rid]
  # vector of province_screen responses with the same IP address
  provinces <- df$province_screen[df$record_id %in% rid]
  
  # if all of the min and max of the above 3 vectors are equal
        # note: i don't believe this code is technically necessary
  if (max(ages) == min(ages) 
      & max(cannabis) == min(cannabis) 
      & max(provinces) == min(provinces)) {
        # removes flag for exclusion (shouldn't be flagged anyway)
        df$ip_duplicate[df$record_id %in% rid] <- 0
  } else { # if any of the min and max of the above 3 vectors are not equal
        # flags for duplicate IP and exclusion
        df$ip_duplicate[df$record_id %in% rid] <- 1
  }  
  
}

rm(IPs, ages, cannabis, provinces, IP, rid, duplicate_IPs)
```
...
IPs <- read.xlsx("SSMRT Participant Info.xlsx", sheet = "IPs") # reading IP address file
...
df$province_screen_label <- dplyr::recode(df$province_screen, 
                                   '1' = 'British Columbia',
                                   '2' = 'Alberta',
                                   '3' = 'Saskatchewan',
                                   '4' = 'Manitoba',
                                   ...
                                   '15' = 'None of the above - I live elsewhere'
                                   )
...
df$exclude[
  ...
  | is.na(df$entrance_timestamp) | df$rct_dq_complete == 2
  # catching those who didn't make it to the end
  | is.na(df$exit_timestamp)
] <- 1

# un-excludes participants who we have decided to manually keep
df$exclude[df$manual_check == 2 & df$manual_decided == 0] <- 0

# removes manual_check flag for participants who are already excluded
df$manual_check[df$exclude == 1] <- 0
df$manual_decided[df$manual_check == 0] <- 0

```

# Followup Tracking

The code below reads a file of followup survey tracking information. Keeps track of the dates of follow-ups and if/when those follow-ups were completed. Contains 3 columns for each follow-up point:

-   send_by: the date at which follow-ups should be sent.

-   sent: an indicator filled manually, indicating the follow-up was sent. NA indicates it has not yet been sent.

-   done: the date at which the follow-up was completed by the participants. NA indicates it has not yet been completed.

```{r followup}

# reading file of tracking information
followups <- read.xlsx("SSMRT Participant Info.xlsx", sheet = "Followup tracking")

# for each valid/un-excluded access code, add these as a new row to the tracking file if they are not already in the tracking file
for (AC in df$access_code[df$exclude == 0]){
  if (AC %notin% followups$access_code){
    followups[nrow(followups)+1, 1] <- AC
  }
}

# for the access codes with blank "send by" dates, add them
# calculated as exit date + 14 days (or + 28 days for F2)
for (AC in followups$access_code[is.na(followups$f1_send_by)]) {
  followups$f1_send_by[followups$access_code == AC] <- 
    as.character(date(df$entrance_timestamp[df$access_code == AC & df$exclude == 0])+14)
}

for (AC in followups$access_code[is.na(followups$f2_send_by)]) {
  followups$f2_send_by[followups$access_code == AC] <- 
    as.character(date(df$entrance_timestamp[df$access_code == AC & df$exclude == 0])+28)
}

# if data exists for F1,
# for each access code with blank "done" columns,
# checks if F1 has been completed
# if completed, adds completion date to tracking file
# same code repeated for F2 below
if ("exit_timestamp_F1" %in% names(df)) {
  for(AC in followups$access_code[is.na(followups$f1_done)]) {
    if (TRUE != all.equal(as.POSIXct(NULL, tz = "UTC"), 
                  (df$exit_timestamp_F1[df$access_code == AC 
                                    & !is.na(df$access_code)
                                    & df$exclude == 0]))) {
      followups$f1_done[followups$access_code == AC] <- 
        as.character(date(df$exit_timestamp_F1[df$access_code == AC & !is.na(df$access_code) & df$exclude == 0]))
    }
  }
}

if ("exit_timestamp_F2" %in% names(df)) {
  for(AC in followups$access_code[is.na(followups$f2_done)]) {
    if (TRUE != all.equal(as.POSIXct(NULL, tz = "UTC"), 
                  (df$exit_timestamp_F2[df$access_code == AC 
                                    & !is.na(df$access_code)
                                    & df$exclude == 0]))) {
      followups$f2_done[followups$access_code == AC] <- 
        as.character(date(df$exit_timestamp_F2[df$access_code == AC & !is.na(df$access_code)]))
    }
  }
}

# removing people who are manually excluded if they are present in the sheet
for (AC in followups$access_code) {
  if (all(df$exclude[df$access_code == AC & !is.na(df$access_code)] == 1)) {
    followups <- followups[followups$access_code != AC,]
  }
}
...
The code below serves to keep track of who has been sent compensation, and who has been sent responses letting them know their submission was rejected. It also updates the file with new users who will need to be sent compensation, and their choice of compensation.

```{r compensation}
## TODO: this is fragile - duplicate ACs in df will break this
compensation <- read.xlsx("SSMRT Participant Info.xlsx", sheet = "Compensation logs")

# finding access codes missing from the compensation logs and adding them
for (AC in df$access_code[!is.na(df$giftcard)]) {
  if (AC %notin% compensation$access_code) {
    compensation[nrow(compensation)+1, 1] <- AC
  }
}

# filling out email addresses for access codes in the compensation file
for (AC in compensation$access_code[is.na(compensation$email)]) {
  compensation$email[compensation$access_code == AC] <- 
    unique(df$email_end[df$access_code == AC 
                 & !is.na(df$access_code) & !is.na(df$email_end)])
}

# if participants have had their data rejected, make note of this in the compensation file
for (AC in compensation$access_code) {
  if (all(df$exclude[df$access_code == AC & !is.na(df$access_code)] == 1)) {
    compensation$approved[compensation$access_code == AC] <- "Rejected"
  } else {
    compensation$approved[compensation$access_code == AC] <- ""
  }
}

# filling out preferred giftcards for access codes in the compensation file
for (AC in compensation$access_code[is.na(compensation$card_type)]) {
  compensation$card_type[compensation$access_code == AC] <- 
    unique(df$giftcard[df$access_code == AC 
                & !is.na(df$access_code) & !is.na(df$giftcard)])
}

# recoding card type to be labels and not numbers
compensation$card_type <- dplyr::recode(compensation$card_type,
                                        '1' = "Amazon.ca",
                                        '2' = "PC",
                                        '3' = "None",
                                        '4' = "Walmart")
...
### The code below is the same as the above, but for F! and F2
# filling out preferred giftcards for access codes in the compensation file
for (AC in compensation$access_code[is.na(compensation$f1_card_type)]) {
  if (any(!is.na(df$giftcard_F1[df$access_code == AC 
                            & !is.na(df$access_code)]))) {
    if (any(df$exclude[df$access_code == AC & !is.na(df$access_code)] == 0)) {
          compensation$f1_card_type[compensation$access_code == AC] <- 
      unique(df$giftcard_F1[df$access_code == AC 
                  & !is.na(df$access_code) 
                  & !is.na(df$giftcard_F1)])
    }
    if (all(df$exclude[df$access_code == AC & !is.na(df$access_code)] == 1)) {
          compensation$f1_card_type[compensation$access_code == AC] <- 
      "Rejected"
...
# recoding card type to be labels and not numbers
compensation$f1_card_type <- dplyr::recode(compensation$f1_card_type,
                                        '1' = "Amazon.ca",
                                        '2' = "PC",
                                        '3' = "None",
                                        '4' = "Walmart")
# F2
for (AC in compensation$access_code[is.na(compensation$f2_card_type)]) {
  if (any(!is.na(df$giftcard_F2[df$access_code == AC 
                            & !is.na(df$access_code)]))) {
    if (any(df$exclude[df$access_code == AC & !is.na(df$access_code)] == 0)) {
          compensation$f2_card_type[compensation$access_code == AC] <- 
      unique(df$giftcard_F2[df$access_code == AC 
                  & !is.na(df$access_code) 
                  & !is.na(df$giftcard_F2)])
    }
    if (all(df$exclude[df$access_code == AC & !is.na(df$access_code)] == 1)) {
          compensation$f2_card_type[compensation$access_code == AC] <- 
      "Rejected"
...
# recoding card type to be labels and not numbers
compensation$f2_card_type <- dplyr::recode(compensation$f2_card_type,
                                        '1' = "Amazon.ca",
                                        '2' = "PC",
                                        '3' = "None",
                                        '4' = "Walmart")
...
# saving for writing to file later
COMPENSATION_LOGS <- compensation
...
The code below fetches information regarding participants' usage of the SSMRT web platform. This information includes:
...
This section is only run if the Google Analytics parameters are provided within the keys.R file.
...
if (exists("GA_TOKEN")) {
  ...
  # authenticating
  ga_auth(token = GA_TOKEN, email = "crism.cannabis@gmail.com")
  
  #meta <- ga_meta("data", propertyId = GA_TOKEN)
  
  analytics_data <- subset(df, select = c("access_code", "group_F1", "group_F2", "entrance_timestamp", "exclude"))
  analytics_data <- subset(analytics_data, exclude == 0)
  analytics_data <- subset(analytics_data, group_F1 == "Intervention" | group_F2 == "Intervention")
  
  
  analytics_data$exclude <- NULL
  analytics_data$group_F1 <- NULL
  analytics_data$group_F2 <- NULL
  
  analytics_data <- rename(analytics_data, starttime = entrance_timestamp)
  # formatting starttime to POSIX
  analytics_data$starttime <- strptime(analytics_data$starttime, format = "%Y-%m-%d %H:%M:%S")
  # removing the time, leaving only date
  analytics_data$starttime <- round_date(analytics_data$starttime, "day")
  
  # initializing these columns of google analytics data
  analytics_data$first_visit <- NA
  analytics_data$page_view <- NA
  analytics_data$screen_view <- NA
  analytics_data$session_start <- NA
  analytics_data$user_engagement <- NA
  analytics_data$UserEngagementDuration <- NA
  
  for (AC in analytics_data$access_code) {
    filter <- ga_data_filter("customUser:current_user"==AC)
    D1 <- analytics_data$starttime[analytics_data$access_code == AC] - ddays(1)
    D2 <- D1 + ddays(30)
    
    dat <- try(ga_data(
      propertyId = 330608169,
      metrics = c("eventCount", "userEngagementDuration"),
      date_range = c(D1, D2), # for the x-day window
      #date_range = c("2020-01-01", "2024-04-23"), # for "all time"
    ...
    if (exists("dat") & all(class(dat) != "try-error")) {
      ...
      analytics_data$UserEngagementDuration[analytics_data$access_code == AC] <- duration
      
      if ("first_visit" %in% dat$eventName) {
        FV <- dat$eventCount[dat$eventName == "first_visit"]
        analytics_data$first_visit[analytics_data$access_code == AC] <- FV}
      
      if ("page_view" %in% dat$eventName) {
        PV <- dat$eventCount[dat$eventName == "page_view"]
        analytics_data$page_view[analytics_data$access_code == AC] <- PV}
      
      if ("screen_view" %in% dat$eventName) {
        SV <- dat$eventCount[dat$eventName == "screen_view"]
        analytics_data$screen_view[analytics_data$access_code == AC] <- SV}
      
      if ("session_start" %in% dat$eventName) {
        SS <- dat$eventCount[dat$eventName == "session_start"]
        analytics_data$session_start[analytics_data$access_code == AC] <- SS}
      
      if ("user_engagement" %in% dat$eventName) {
        UE <- dat$eventCount[dat$eventName == "user_engagement"]
        analytics_data$user_engagement[analytics_data$access_code == AC] <- UE}
      
      suppressWarnings(rm(FV, PV, SS, SV, UE, duration, GA_TOKEN))
    }
  }
  write.csv(analytics_data, "GA Data.csv", row.names = FALSE)
}
...
The code below keeps a running tally of the number of eligible participants each time it is run. This sheet is shared on GitHub and also posted on Google Sheets in [Google reporting], where it is more up-to-date.

```{r n}
# if the recruitment number file exists
if (file.exists('SSMRT Participant Info.xlsx')) {
  # reading recruitment numbers file
  RECRUITMENT_NUMBERS <- read.xlsx("SSMRT Participant Info.xlsx", sheet = "N")
  
  # if today isn't in the recruitment numbers file
  if (as.character(Sys.Date()) %notin% RECRUITMENT_NUMBERS$date) {
  ...
  if (as.character(Sys.Date()) %in% RECRUITMENT_NUMBERS$date) {
...
The code below creates tables that will be posted in the HTML file, as well as posted to Google Sheets in [Google reporting].
...
## Subset of df to only include "included" participants
FINAL_DATA <- subset(df, exclude == 0)


## Creating temporary string versions of some variables needed

# Province
# note that the province factor variable is also created in the section "inconsistency checks"; code copied here
FINAL_DATA$province_screen_label <- dplyr::recode(FINAL_DATA$province_screen, 
                                   '1' = 'British Columbia',
                                   '2' = 'Alberta',
                                   '3' = 'Saskatchewan',
                                   '4' = 'Manitoba',
...
# Gender
FINAL_DATA$gender_label <- dplyr::recode(FINAL_DATA$gender,
                                  '1'	= 'Male',
                                  '2' =	'Female',
                                  '3' =	'Neither male nor female',
                                  '4' =	'Indigenous or other gender identity',
                                  '5' =	'Other',
                                  '6' =	'Prefer not to say'
                                  )

# Race
FINAL_DATA$race_label <- dplyr::recode(FINAL_DATA$race,
                                '1' =	'White',
                                '2' =	'South Asian',
                                '3' =	'Black',
                                '4' =	'Filipino/Pacific Islander',
                                '5' =	'Latin American',
                                '6' =	'Indigenous',
                                '7' =	'South East Asian',
                                '8' =	'Arab',
...
# Income
FINAL_DATA$income_label <- dplyr::recode(FINAL_DATA$incom,
                                  '1'	= 'Less than $20,000',
...
# Education
FINAL_DATA$education_label <- dplyr::recode(FINAL_DATA$edu,
...
# Invitation source
FINAL_DATA$invite_label <- dplyr::recode(FINAL_DATA$invite,
                                  '1'	= 'Contacted directly through email',
                                  '2'	= 'Advertisement on Facebook',
                                  '3'	= 'News advertisement',
                                  '4'	= 'Advertisement on Instagram',
                                  '5'	= 'Other'
                                )


vars <- c('age_screen', 'gender_label', 'race_label', 'income_label', 'education_label', 'invite_label', 'province_screen_label')

# applying labels (for better representation in the table)
FINAL_DATA <- apply_labels(FINAL_DATA,
                           #age_screen = "Age", This breaks the next function, so I rename it manually later
                           gender_label = "Gender",
                           race_label = "Race",
                           income_label = "Income",
                           education_label = "Education",
                           invite_label = "Invitation source",
                           province_screen_label = "Province")


# TODO: add p = TRUE argument here once recruitment begins; sample too small for now
if ("group_F1" %in% names(df)){
  demographic_table <- summary_factorlist(FINAL_DATA, "group_F1", vars, na_include = TRUE)
  
  demographic_table$label[demographic_table$label == "age_screen"] <- "Age"
  
  google_info <- subset(FINAL_DATA, select = c(age_screen, gender_label, income_label, education_label, invite_label, province_screen_label))

  google_info[is.na(google_info)] <- "Missing"
  
  google_info$age_screen <- sort(google_info$age_screen)
  google_info$gender_label <- sort(google_info$gender_label)
  google_info$income_label <- sort(google_info$income_label)
  google_info$education_label <- sort(google_info$education_label)
  google_info$invite_label <- sort(google_info$invite_label)
  google_info$province_screen_label <- sort(google_info$province_screen_label)
  
  google_info$age_screen <- as.numeric(google_info$age_screen)
  
  kable(demographic_table)
}
### Removing unneeded environment variables
rm(vars)

### Removing temporary variables
FINAL_DATA$province_screen_label <- NULL
FINAL_DATA$gender_label <- NULL
FINAL_DATA$race_label <- NULL
FINAL_DATA$income_label <- NULL
FINAL_DATA$education_label <- NULL
FINAL_DATA$invite_label <- NULL

```

## Google reporting

Reporting the above sections to Google Sheets.

```{r google_sheets}

# if the USER_EMAIL and DOC_ID variables exist (if not; section skipped)
if (exists("USER_EMAIL") & exists("DOC_ID") & exists("google_info")) {
  ...
  ### Tracking study total eligible N
  #reading online data
  online_N <- read_sheet(ss = DOC_ID, sheet = "N")
  
  # if today's date is in the online data
  if (as.character(Sys.Date()) %in% online_N$date) {
    # updating associated N in the sheet
    online_N$N[online_N$date == as.character(Sys.Date())] <- 
      RECRUITMENT_NUMBERS$N[RECRUITMENT_NUMBERS$date == as.character(Sys.Date())]
    
    # writing to online tracking document
    sheet_write(online_N,
              ss = DOC_ID,
              sheet = "N")
    
  } else { # if today's date is NOT in the sheet
    
    # creating a dataframe with today's date and eligible N
    newN <- data.frame(date = as.character(Sys.Date()),
                         N = length(df$record_id[df$exclude == 0])
    )
    
    # appending to online sheet
    sheet_append(ss = DOC_ID,
                 sheet = "N",
                 newN)
  ...
  ### Writing demographic table to Google Sheets
  sheet_write(google_info,
              ss = DOC_ID,
              sheet = "Demographics")
  
  ### Removing unneeded variables
  rm(demographic_table, online_N, DOC_ID, USER_EMAIL)
}
...
write.csv(FINAL_DATA, paste(todays_folder_name, "SSMRT dataFinal.csv", sep = ""), row.names = FALSE)

# writing location data to file
write.xlsx(LOCATIONS, "SSMRT Location Data.xlsx")

# writing participant info to file
write.xlsx(list("Followup tracking" = FOLLOWUPS,
                "N" = RECRUITMENT_NUMBERS,
                "Manual decisions" = MANUAL_DECISIONS,
                "IPs" = IPS,
                "Compensation logs" = COMPENSATION_LOGS
                ), "SSMRT Participant Info.xlsx", rowNames = FALSE)
